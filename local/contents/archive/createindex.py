...
def execStage(c):
  res = c.execute("select * from stage inner join hall on stage.place = hall.id")
  if (res):
    text = []
    add_str(text, '-inherit archiveindex.baseslim')
    ...
    for i in res:
      ...
      add_str(text, 'h2')
      add_str(text, '  a href="/archive/' + str(concertpagename) + '.html"')
      add_str(text, '    ' + concertname)
      add_str(text, '.pure-g')
      add_str(text, '  .pure-u-1.pure-u-md-1-2.pure-u-lg-2-4 ' + concertdate + concerttime)
      add_str(text, '  .pure-u-1.pure-u-md-1-2.pure-u-lg-2-4 ' + concertplace)
...
#     #print(text)
    ...
    with open(outputdir + 'index' + '.slim', 'w') as fp:
     fp.write('\n'.join(text))
     fp.close()
